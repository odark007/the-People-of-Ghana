...
const TIER_ICONS: Record<AnonymityLevel, string> = { L1: "🕶️", L2: "🎭", L3: "🙋" };
...
export default function ProfileClient({ userId, email, profile, stats }: ProfileClientProps) {
  ...
  async function saveLevel() {
    ...
    if (!res.ok) {
      setSaveError(data.error?.message ?? "Failed to save. Please try again.");
      return;
    }
    ...
    <div className="pb-10">
      ...
      <div className="bg-ghana-black px-5 pt-8 pb-8 text-center">
        ...
        <div className={`w-20 h-20 rounded-full mx-auto mb-3 flex items-center justify-center
                         text-3xl font-bold text-white border-3 ${TIER_BORDER[currentLevel]}
        ...
        {/* Email — shown muted, never public */}
      ...
      <div className="grid grid-cols-3 gap-px bg-[var(--border)] mx-4 mt-5 rounded-2xl overflow-hidden shadow-card">
        {[
          ...
          { num: profile.role === "admin" || profile.role === "superadmin" ? "✓" : "—", label: "Admin" },
        ].map((s) => (
      ...
      <div className="mx-4 mt-5">
        ...
        <div className="card overflow-hidden">
          ...
          <div className="p-4 flex items-center gap-3">
            <span className="text-2xl">{TIER_ICONS[currentLevel]}</span>
      ...
      <div className="mx-4 mt-5">
        ...
        <div className="card overflow-hidden divide-y divide-[var(--border)]">
          {[
            { icon: "📋", label: "My Reports",     href: `/feed?user=${userId}` },
            { icon: "🏛️", label: "Browse Leaders", href: "/directory"           },
            { icon: "📢", label: "Report Issue",   href: "/reports/new"          },
            ...(profile.role === "admin" || profile.role === "superadmin"
              ? [{ icon: "⚙️", label: "Admin Panel", href: "/admin/moderation" }]
              : []),
          ].map(({ icon, label, href }) => (
            <a
              key={label}
      ...
      {showLevelEdit && (
        ...
        >
          ...
          >
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-lg">{TIER_ICONS[level]}</span>
                      <span className="font-bold text-sm">{cfg.label}</span>
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-[var(--surface-2)] text-[var(--text-muted)]">
                        {level}
                      </span>
                      {selected && (
                        <div className="ml-auto w-5 h-5 rounded-full bg-ghana-black flex items-center justify-center">
                          <div className="w-2 h-2 rounded-full bg-white" />
                        </div>
                      )}
                    </div>
                    <p className="text-xs text-[var(--text-muted)] leading-relaxed">{cfg.description}</p>
                  </button>
                );
              })}
            </div>
            ...
            {saveError && (
              <p className="text-ghana-red text-xs mb-3">{saveError}</p>
            )}
