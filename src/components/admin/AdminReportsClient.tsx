  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-[var(--text-subtle)]">
                    ...
                    {REPORT_CATEGORY_LABELS[r.category as keyof typeof REPORT_CATEGORY_LABELS]} ·{" "}
      ...
      {selected && (
        ...
        >
          <div
            className="bg-white w-full max-w-lg rounded-t-3xl md:rounded-2xl p-6
                       max-h-[85vh] overflow-y-auto animate-slide-up"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-start justify-between mb-4">
              <h2 className="font-bold text-lg leading-tight flex-1 pr-4">
                {selected.title}
              </h2>
              <button
                onClick={() => setSelected(null)}
                className="text-[var(--text-subtle)] hover:text-black flex-shrink-0"
              >
                ✕
              </button>
            </div>
            ...
            {selected.image_url && (
              <img
                ...
                alt=""
                className="w-full h-44 object-cover rounded-xl mb-4 border border-[var(--border)]"
              />
            )}
            ...
            <div className="grid grid-cols-2 gap-3 mb-4 text-xs">
              {[
                {
                  label: "Category",
                  value: REPORT_CATEGORY_LABELS[selected.category as keyof typeof REPORT_CATEGORY_LABELS],
                },
                {
                  label: "Reporter",
                  value: `${selected.reporter_public_name} (${selected.reporter_anonymity_level})`,
                },
                {
                  label: "Location",
                  value: selected.district?.name ?? selected.region?.name ?? "Unknown",
                },
                {
                  label: "Submitted",
                ...
                <div key={label} className="bg-[var(--surface-2)] rounded-lg p-3">
                  <p className="text-[var(--text-subtle)] uppercase tracking-wide font-bold text-[10px] mb-0.5">
                    {label}
                  </p>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 py-3 rounded-xl border-2 border-ghana-red/30 text-ghana-red
                           ...
                           disabled:opacity-40"
              >
                {isDeleting ? "Deleting…" : "🗑️ Delete + Purge Image"}
              </button>
              <button
