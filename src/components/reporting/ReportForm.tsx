...
export default function ReportForm() {
  ...
  function detectGps() {
    ...
    setGps({ latitude: null, longitude: null, status: "detecting",
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        setGps({ latitude: null, longitude: null, status: "error",
          label: err.code === 1
            ? "Location permission denied. Select your region manually."
            : "Could not detect GPS." });
        loadRegions();
      },
  ...
  function validate(): boolean {
    ...
    if (!imageUrl)                      e.image       = isUploading
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        setErrors({ submit: data.error?.message ?? "Submission failed. Please try again." });
        return;
      }
  ...
  if (submitted) {
    return (
      <div className="flex flex-col items-center justify-center px-6 py-20 text-center animate-fade-in">
        <div className="w-20 h-20 rounded-full bg-ghana-green/10 flex items-center justify-center text-4xl mb-5">
          ✅
        </div>
  ...
  return (
    <form onSubmit={handleSubmit} className="px-4 pb-8 flex flex-col gap-5 mt-5" noValidate>
          ...
          {CATEGORIES.map(([key, label]) => (
            <button
              ...
              className={`
                ...
                ${category === key
                  ? "border-ghana-red bg-ghana-red/5 scale-[1.02]"
      ...
      <div>
        ...
        {imagePreview ? (
          <div className="relative rounded-2xl overflow-hidden border-2 border-[var(--border)]">
            ...
            {!isUploading && imageUrl && (
              <div className="absolute top-3 right-3 bg-ghana-green text-white text-xs font-bold px-3 py-1 rounded-full">
                ✓ Uploaded
              </div>
            )}
          ...
          <button
            ...
            className={`
              ...
              ${errors.image || uploadError
                ...
                : "border-[var(--border)] hover:border-ghana-red"}
            `}
          >
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-ghana-green mt-1.5 font-medium">
        ...
        {uploadError && (
          <p className="text-ghana-red text-xs mt-1">{uploadError}</p>
        )}
        {errors.image && !uploadError && (
          <p className="text-ghana-red text-xs mt-1">{errors.image}</p>
        )}
      </div>
      ...
      <div>
          ...
          <div className="flex flex-col gap-3">
            <div className="flex items-start gap-3 p-4 bg-amber-50 rounded-2xl border border-amber-200">
              ...
              <div>
                ...
                <button
                  type="button"
                  onClick={detectGps}
                  className="text-xs text-ghana-green hover:underline mt-1"
                >
                  Try GPS again
                </button>
              </div>
            ...
            {regions.length > 0 && (
              ...
              >
                ...
                {regions.map((r: any) => (
                  <option key={r.id} value={r.id}>{r.name} Region</option>
                ))}
              </select>
            )}
          </div>
        ...
        {errors.location && (
          <p className="text-ghana-red text-xs mt-1">{errors.location}</p>
        )}
      </div>

      {/* Privacy note */}
      <div className="px-4 py-3 bg-[var(--surface-2)] rounded-xl border border-[var(--border)]">
